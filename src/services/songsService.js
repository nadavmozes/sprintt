import { storageService } from "./songsStorage.js";
...
var songs = _createSongs()

function getById(songId) {
  const song = songs.find(song => song.playlist_id === songId)
...
function _createSongs() {
  var songsFromStorage = storageService.loadFromStorage(KEY)
  if (!songsFromStorage || !songsFromStorage.length) {
    songsFromStorage = [
      {
  ...
  _saveSongsToStorage(songsFromStorage);
  return songsFromStorage
}


function _saveSongsToStorage(songsFromStorage = songs) {
  storageService.saveToStorage(KEY, songsFromStorage)
}
